...
#					return []
	...
	def handleUnitEditExperienceCB (self, argsList) :
		...
		self.m_pActivePlot.getUnit(self.m_iCurrentUnit).setExperienceTimes100(iNewXP,-1)
		return 1
	...
	def handleCitySettlementButton (self, argsList) :
		if not self.m_pActivePlot.isCity():
			return 1
		city = self.m_pActivePlot.getPlotCity()
		if (city):
	...
	def handlePromotionCB ( self, iNewPromotion ) :
		...
		if bOn:
			self.m_pActivePlot.getUnit(self.m_iCurrentUnit).setHasPromotion(iNewPromotion,true)
		else:
			self.m_pActivePlot.getUnit(self.m_iCurrentUnit).safeRemovePromotion(iNewPromotion)
		return 1
...
#			if (not self.m_pCurrentPlot.isAdjacentNonrevealed(iActiveTeam) and self.m_pCurrentPlot.isRevealed(iActiveTeam, false)):
	...
	def setUnitEditInfo(self, bSamePlot):
		...
		self.m_tabCtrlEdit.addSectionSpinner(
			...
			100000.0,
			1.0,
			self.m_pActivePlot.getUnit(self.m_iCurrentUnit).getExperienceTimes100(),
			0,
			0)
	...
	def setCityEditInfo(self):
		...
		self.m_tabCtrlEdit.addSectionCheckbox(localText.getText("TXT_KEY_WB_SETTLEMENT",()), "CvScreensInterface", "WorldBuilderHandleCitySettlementButton", "CitySettlementButton", 0, self.m_pActivePlot.getPlotCity().isSettlement())
	...
	def setEditButtonClicked(self, argsList):
		...
		if (self.m_bUnitEditCtrl):
			...
			if bOn:
				self.m_pActivePlot.getUnit(self.m_iCurrentUnit).setHasPromotion(iIndex,true)
			else:
				self.m_pActivePlot.getUnit(self.m_iCurrentUnit).safeRemovePromotion(iIndex)
