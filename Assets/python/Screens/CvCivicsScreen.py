...
class CvCivicsScreen:
	...
	def __init__(self):
		...
		self.HELP_LEFT = 290
		self.HELP_RIGHT = 950 # y coordinate for the right limit of the civic help background (brown panel)
		self.BUTTON_SIZE = 24
	...
	def drawAllButtons(self):
		...
		for i in range(gc.getNumCivicOptionInfos()):
			...
			iCount = 0
			for j in range(gc.getNumCivicInfos()):
				pCivic = gc.getCivicInfo(j)
				if (pCivic.getCivicOptionType() == i) and (pCivic.getPrereqCivilization() == -1 or pCivic.getPrereqCivilization() == pPlayer.getCivilizationType()):
					# hide religious civics if agnostic or intolerant
					# if not ((pPlayer.isIntolerant() or pPlayer.isAgnostic()) and (pCivic.isStateReligion() or pCivic.isNoNonStateReligionSpread())):
					...
					screen.addCheckBoxGFCAt("CivicList", self.getCivicsButtonName(j), gc.getCivicInfo(j).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), fX + 12, fY, self.BUTTON_SIZE, self.BUTTON_SIZE, WidgetTypes.WIDGET_HELP_CIVIC, j, 1, ButtonStyles.BUTTON_STYLE_LABEL, False)
					screen.setActivation( self.getCivicsButtonName(j), ActivationTypes.ACTIVATE_NORMAL )
					screen.hide (self.getCivicsButtonName(j) )

					screen.addCheckBoxGFCAt(self.QUICKVIEW_ADD, self.getCivicsButtonName(j)+self.QUICKVIEW_ADD, gc.getCivicInfo(j).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), ffX, ffY, self.BUTTON_SIZE, self.BUTTON_SIZE, WidgetTypes.WIDGET_HELP_CIVIC, j, 1, ButtonStyles.BUTTON_STYLE_LABEL, False)
		...
		for pUnit in py.getUnitList():
			if gc.getUnitInfo(pUnit.getUnitType()).isAbandon():
				if gc.getUnitInfo(pUnit.getUnitType()).getPrereqCivic() == iCivicPrev:
					bWarn = True
		if bWarn:
			addPopup(CyTranslator().getText("TXT_KEY_POPUP_CIVIC_ABANDON_WARN",()), 'art/interface/popups/Abandon.dds')
	...
	def drawHelpText(self, iCivicOption):
		...
		szHelpText = CyGameTextMgr().parseCivicInfoHelp(iCivic, False, True, True)
		HelpList = szHelpText.splitlines()
		#Show AI values if using debug mode
		if (CyGame().isDebugMode()):
			HelpList.append("AI value: %d" %activePlayer.AI_civicValue(iCivic))
		i = 0
		for item in HelpList:
	...
	def Revolution(self, inputClass):
		...
		if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED) :
			if (activePlayer.canRevolution(0)):
				...
				if activePlayer.isCivic(CvUtil.findInfoTypeNum(gc.getCivicInfo,gc.getNumCivicInfos(),'CIVIC_REPUBLIC')):
					cf.resetRepublicTraits(activePlayer)
			screen = self.getScreen()
			screen.hideScreen()
