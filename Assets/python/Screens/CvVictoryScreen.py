...
class CvVictoryScreen:
	...
	def showGameSettingsScreen(self):
		...
		for i in range(GameOptionTypes.NUM_GAMEOPTION_TYPES):
			if not i == GameOptionTypes.GAMEOPTION_NO_ESPIONAGE or i > GameOptionTypes.GAMEOPTION_LIMITED_CIVILIZATION_SELECTION:
	...
	def showVictoryConditionScreen(self):
		...
		ourEvil = pActivePlayer.getGlobalCounterContrib()

		iMostEvilOtherPlayer = -1
		iMostEvilOtherTotalContrib = 0

		for iLoopPlayer in range(gc.getMAX_PLAYERS()):
			pLoopPlayer = gc.getPlayer(iLoopPlayer)
			if pLoopPlayer.isAlive() and iLoopPlayer != pActivePlayer.getID():
				if pLoopPlayer.getGlobalCounterContrib() > iMostEvilOtherTotalContrib:
					iMostEvilOtherPlayer = iLoopPlayer
					iMostEvilOtherTotalContrib = pLoopPlayer.getGlobalCounterContrib()
		...
		for iLoop in range(CyMap().getNumAreas()):
			...
			if gc.getGame().isVictoryValid(iLoopVC):
				...
				if (victory.getReligionPercent() > 0):
					...
					if (iOurReligion != -1):
						...
						screen.setTableText(szTable, 2, iRow, activePlayer.getTeam().getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
						screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_NO_HOLY_CITY", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					if (iBestReligion != -1):
				...
				if (iLoopVC == gc.getInfoTypeForString('VICTORY_GONE_TO_HELL')):
					iRow = screen.appendTableRow(szTable)
					screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_PERCENT_HELL_TERRAIN", (int((100.0 * iEvilTiles) / iTotalTiles), gc.getDefineINT("GONE_TO_HELL_THRESHOLD_PERCENTAGE"),)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					iRow = screen.appendTableRow(szTable)
					screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_AC_CONTRIBUTION",()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					screen.setTableText(szTable, 2, iRow, activePlayer.getTeam().getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					screen.setTableText(szTable, 3, iRow, unicode(ourEvil), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					if (iMostEvilOtherPlayer != -1):
						screen.setTableText(szTable, 4, iRow, gc.getPlayer(iMostEvilOtherPlayer).getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
						screen.setTableText(szTable, 5, iRow, unicode(iMostEvilOtherTotalContrib), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					bEntriesFound = True
