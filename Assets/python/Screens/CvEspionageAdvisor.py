	...
	def loadScenario(self, szScenarioName):
		if not gc.getGame().isOption(GameOptionTypes.GAMEOPTION_WB_LOAD_SCREEN):
			...
			for iPlayer in range(gc.getMAX_PLAYERS()):
				...
				if pPlayer.isAlive():
					if pPlayer.isHuman():
						addPlayerPopup(CyTranslator().getText("TXT_KEY_SCENARIO_WARNING",()), iPlayer)
			return 0
		fileName = os.path.join(os.getcwd(), "Mods", "Ashes of Erebus", "Assets", "XML", "Scenarios", szScenarioName)
