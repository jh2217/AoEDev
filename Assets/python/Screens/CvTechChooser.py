			...
			for j in range( gc.getNumUnitClassInfos() ):
				...
				if (eLoopUnit != -1):
					if (gc.getUnitInfo(eLoopUnit).getPrereqAndTech() == i):
						...
						screen.addDDSGFCAt( szUnitButton, szTechRecord, newbutton, iX + fX, iY + Y_ROW, TEXTURE_SIZE, TEXTURE_SIZE, WidgetTypes.WIDGET_PEDIA_JUMP_TO_UNIT, eLoopUnit, 1, True )
						fX += X_INCREMENT
			...
			if (not gc.isNoCrash()):
				...
				for j in xrange( gc.getNumTraitInfos() ):
					...
					if (bFirstToTech and iTraitPointsFirstTo != 0) or iTraitPoints != 0:
						...
						screen.addDDSGFCAt( szTraitButton, szTechRecord, szButton, iX + fX, iY + Y_ROW, TEXTURE_SIZE, TEXTURE_SIZE, WidgetTypes.WIDGET_HELP_TRAITCOUNTER, j, i, True )
						fX += X_INCREMENT
					...
					if iTemporaryTrait > 0 or (iTemporaryTraitFirstTo > 0 and bFirstToTech):
						...
						screen.addDDSGFCAt( szTraitButton, szTechRecord, szButton, iX + fX, iY + Y_ROW, TEXTURE_SIZE, TEXTURE_SIZE, WidgetTypes.WIDGET_HELP_TEMPORARY_TRAIT_GAIN, j, i, True )
						fX += X_INCREMENT
			...
			for j in xrange ( gc.getNumSpellInfos() ):
				if (gc.getSpellInfo(j).getTechPrereq() == i) and (gc.getSpellInfo(j).isGlobal() or gc.getSpellInfo(j).isCityAction()):
					if (gc.getSpellInfo(j).getCivilizationPrereq() == -1 or gc.getSpellInfo(j).getCivilizationPrereq() == gc.getGame().getActiveCivilizationType()):
						...
						screen.addDDSGFCAt( szCityActionButton, szTechRecord, szButton, iX + fX, iY + Y_ROW, TEXTURE_SIZE, TEXTURE_SIZE, WidgetTypes.WIDGET_PEDIA_JUMP_TO_SPELL, j, i, False )
						fX += X_INCREMENT
