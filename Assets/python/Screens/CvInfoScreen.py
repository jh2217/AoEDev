...
############################################### STATISTICS ###############################################
	...
	def initText(self):
		...
		self.TEXT_BUILT_UNIT = localText.getText("TXT_KEY_INFO_SCREEN_BUILT_UNIT", ())
		self.TEXT_BUILT_BUILDING = localText.getText("TXT_KEY_INFO_SCREEN_BUILT_BUILDING", ())
...
#		self.bShowAllPlayers = false
	...
	def showScreen(self, iTurn, iTabID, iEndGame):
		...
		screen.addDDSGFC("DemographicsScreenBackground", ArtFileMgr.getInterfaceArtInfo("INFO_SCREEN").getPath(), 0, 0, self.W_SCREEN, self.H_SCREEN, WidgetTypes.WIDGET_GENERAL, -1, -1 )
	...
	def determineCityData(self):
		...
		for iRankLoop in range(5):
			...
			if (pCity):
				...
				iTurnYear = pCity.getGameTurnFounded()
				...
				if (pCity.isRevealed(gc.getGame().getActiveTeam()) or gc.getTeam(pPlayer.getTeam()).isHasMet(gc.getGame().getActiveTeam())):
					...
					self.szCityDescs[iRankLoop] = ("%s, %s" %(pPlayer.getCivilizationAdjective(0),localText.getText("TXT_KEY_MISC_FOUNDED_IN",(str(iTurnYear),))))
				else:
					self.szCityNames[iRankLoop] = localText.getText("TXT_KEY_UNKNOWN", ()).upper()
					self.szCityDescs[iRankLoop] = ("%s" %(localText.getText("TXT_KEY_UNKNOWN", ()), ))
...
############################################### DISPLAY PROJECT MODE ###############################################
	...
	############################################### DISPLAY WONDER MODE ###############################################
				...
				szSpecialText = CyGameTextMgr().getBuildingHelp(self.iWonderID, True, False, False, None,True)
				splitText = string.split( szSpecialText, "\n" )
				for special in splitText:
					if len( special ) != 0:
						screen.appendListBoxString( listName, special, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
...
################################################### BOTTOM PANEL ###################################################
		...
		iColWidth = int((iChartWidth / 12 * 1))
		screen.setTableColumnHeader(szUnitsTable, 2, self.TEXT_BUILT_UNIT, iColWidth)
		iColWidth = int((iChartWidth / 12 * 1))
		...
		iColWidth = int((iChartWidth / 12 * 1))
		screen.setTableColumnHeader(szBuildingsTable, 1, self.TEXT_BUILT_BUILDING, iColWidth)
