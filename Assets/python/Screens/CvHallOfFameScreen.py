...
class CvHallOfFameScreen:
	...
	def getScreen(self):
		...
		# "Pantheon" and "exit" buttons: horizontal position
		...
		# table dimensions
		self.W_TAB = self.W_SCREEN
		self.H_TAB = self.H_SCREEN - 250 # TODO : softcode value
		
		# we divide the width in 11 (2 for player name, 1 for each other column)
		self.W_COL = int(self.W_TAB/9)
		
		Screen_Pantheon_Ronkhar = CyGInterfaceScreen(self.SCREEN_NAME, self.screenId)
		Screen_Pantheon_Ronkhar.setDimensions(0,0, self.W_SCREEN, self.H_SCREEN)
		return Screen_Pantheon_Ronkhar
	...
	def interfaceScreen(self, bAllowReplay):
		...
		self.EXIT_TEXT = u"<font=4>" + localText.getText("TXT_KEY_EXIT_PANTHEON", ()).upper() + u"</font>"
		...
		screen.addDDSGFC(self.BACKGROUND_ID, ArtFileMgr.getInterfaceArtInfo("SCREEN_BG").getPath(), 0, 0, self.W_SCREEN, self.H_SCREEN, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.addPanel("TechTopPanel", u"", u"", True, False, 0, -2, self.W_SCREEN, 58, PanelStyles.PANEL_STYLE_TOPBAR )
		screen.addPanel( "TechBottomPanel", u"", u"", True, False, 0,  self.H_SCREEN-58, self.W_SCREEN, 58, PanelStyles.PANEL_STYLE_BOTTOMBAR )
		...
		# Header...
		screen.setLabel(self.HEADER_ID, "Background", u"<font=4b>" + localText.getText("TXT_KEY_HALL_OF_FAME_SCREEN_TITLE", ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_MENU, self.Y_TITLE, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
	...
	def drawContents(self):
		screen = self.getScreen()
		screen.addTableControlGFC(self.TABLE_ID, 10, 2, 2 * self.DROPDOWN_SPACING_Y + self.DROPDOWN_Y, self.W_TAB, self.H_TAB, True, True, 16, 16, TableStyles.TABLE_STYLE_STANDARD);
		screen.enableSelect(self.TABLE_ID, False)
		screen.enableSort(self.TABLE_ID)
		screen.setTableColumnHeader(self.TABLE_ID, 0, ""                                                         , 20       )
		# TODO : check how it appears on low resolutions and if ugly, add if W_SCREEN < LIM_VALUE, then hardcode column widths
		screen.setTableColumnHeader(self.TABLE_ID, 1, localText.getText("TXT_KEY_PITBOSS_LEADER",             ()),self.W_COL)
		screen.setTableColumnHeader(self.TABLE_ID, 2, localText.getText("TXT_KEY_NORMALIZED_SCORE",           ()),self.W_COL)
		screen.setTableColumnHeader(self.TABLE_ID, 3, localText.getText("TXT_KEY_HALL_OF_FAME_SORT_BY_DATE",  ()),self.W_COL)
		screen.setTableColumnHeader(self.TABLE_ID, 4, localText.getText("TXT_KEY_GAME_SCORE",                 ()),self.W_COL)
		screen.setTableColumnHeader(self.TABLE_ID, 5, localText.getText("TXT_KEY_CONCEPT_VICTORY",            ()),self.W_COL)
		screen.setTableColumnHeader(self.TABLE_ID, 6, localText.getText("TXT_KEY_PITBOSS_DIFFICULTY",         ()),self.W_COL)
		screen.setTableColumnHeader(self.TABLE_ID, 7, localText.getText("TXT_KEY_HOF_SCREEN_SIZE",            ()),self.W_COL)
		screen.setTableColumnHeader(self.TABLE_ID, 8, localText.getText("TXT_KEY_HOF_SCREEN_STARTING_ERA",    ()),self.W_COL)
		screen.setTableColumnHeader(self.TABLE_ID, 9, localText.getText("TXT_KEY_HOF_SCREEN_GAME_SPEED",      ()),self.W_COL)
