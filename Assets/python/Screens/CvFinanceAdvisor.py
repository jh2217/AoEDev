...
class CvFinanceAdvisor:
	...
	def drawContents(self):
		...
		if (not gc.isNoCrash()):
			totalMercenaryUpkeep=player.getMercenaryUpkeep()
			totalMercenaryGain = player.getMercenaryGains()
		...
		if (not gc.isNoCrash() and totalMercenaryGain > 0):
			yLocation += self.Y_SPACING
			szText = unicode(totalMercenaryGain) + " : " + localText.getText("TXT_KEY_FINANCIAL_ADVISOR_MERCENARY", ())
			screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + localText.getText("TXT_KEY_FINANCIAL_ADVISOR_MERCENARY", ()) + "</font>", CvUtil.FONT_LEFT_JUSTIFY, self.X_INCOME + self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_HELP_FINANCE_FOREIGN_INCOME, self.iActiveLeader, 1)
			screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + unicode(totalMercenaryGain) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_INCOME + self.PANE_WIDTH - self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_HELP_FINANCE_FOREIGN_INCOME, self.iActiveLeader, 1)
			iIncome += totalMercenaryGain
		...
		if (not gc.isNoCrash()):
			...
			screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + unicode(totalMercenaryUpkeep) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_EXPENSES + self.PANE_WIDTH - self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, self.iActiveLeader, 1)
			iExpenses += totalMercenaryUpkeep
		# < Mercenaries End   >
