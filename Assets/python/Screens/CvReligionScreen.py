	...
	def drawReligionInfo(self):
		...
		for i in range(gc.getNumReligionInfos()):
			...
			for i in range(gc.getNumReligionInfos()):
				...
				screen.attachPanelAt( "CivicList", szAreaID, u"", u"", True, True, PanelStyles.PANEL_STYLE_MAIN, self.X_RELIGION, self.Y_RELIGION_AREA, self.X_RELIGION_DIFF + ((i + 2) * self.DX_RELIGION), self.H_RELIGION_AREA, WidgetTypes.WIDGET_GENERAL, i, -1 )
				...
				for pUnit in py.getUnitList():
					if gc.getUnitInfo(pUnit.getUnitType()).isAbandon():
						if gc.getUnitInfo(pUnit.getUnitType()).getStateReligion() == self.iReligionSelected:
							addPopup(CyTranslator().getText("TXT_KEY_POPUP_RELIGION_ABANDON_WARN",()), 'art/interface/popups/Abandon.dds')
#FfH: End Add
