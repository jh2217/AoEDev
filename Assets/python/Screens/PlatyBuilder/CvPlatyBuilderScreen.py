...
## Platy Builder ##
		...
		self.MapMode = ["AddLandMark", "PlotData", "River", "Improvements", "Bonus", "PlotType", "Terrain", "Routes", "Features", "Effects"]
...
## Platy Builder ##
	...
	## Python Effects ##
		...
		elif self.iPlayerAddMode == "Effects":
			self.m_pCurrentPlot.setPlotEffectType(self.iSelection)
	...
	def removeObject( self ):
		...
		if self.iPlayerAddMode == "EraseAll":
			...
			self.m_pCurrentPlot.setPlotEffectType(-1)
		elif self.iPlayerAddMode == "Routes":
			...
			for y in range(Data[2], Data[3]):
				...
				if self.bSensibility and (self.iBrushWidth > 1 or self.iBrushHeight > 1):
					if self.iPlayerAddMode == "Improvements":
						...
						iOldEffect = self.m_pCurrentPlot.getPlotEffectType(-1)
						self.m_pCurrentPlot.setPlotEffectType(-1)
						if self.m_pCurrentPlot.canHavePlotEffect(self.iSelection):
							self.placeObject()
						else:
							self.m_pCurrentPlot.setPlotEffectType(iOldEffect)
					elif self.iPlayerAddMode == "Routes":
	...
	def useLargeBrush(self):
		...
		if self.iPlayerAddMode == "Features":
			return True
		if self.iPlayerAddMode == "Effects":
			return True
		return False
	...
	def refreshSideMenu(self):
		...
		if CyInterface().isInAdvancedStart():
			...
			screen.deleteWidget("AddPlotEffectButton")
...
## Panel Screen ##
			...
			if self.iPlayerAddMode in self.PlayerMode:
				...
				iX = iXStart + 8
				iY += iAdjust
				screen.addCheckBoxGFC("AddPlotEffectButton", "Art/Civs/Sidar/mist.dds", CyArtFileMgr().getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
					 iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 25, ButtonStyles.BUTTON_STYLE_LABEL)
				iX += iAdjust
	...
	def setCurrentModeCheckbox(self):
		...
		screen.setState("AddPlotEffectButton", self.iPlayerAddMode == "Effects")
	...
	def setSelectionTable(self):
		...
		screen.addPanel("SelectionTable", u"", u"", False, False, 0, 0, iWidth, screen.getYResolution(), PanelStyles.PANEL_STYLE_TOPBAR)
		if self.iPlayerAddMode == "Units":
			...
			for item in lItems:
				...
				if self.iSelection == -1:
					self.iSelection = item[1]
				screen.setTableText("WBSelectItem", 0, iRow, "<font=3>" + item[0] + "</font>", gc.getPlotEffectInfo(item[1]).getButton(), WidgetTypes.WIDGET_PYTHON, 7883, item[1], CvUtil.FONT_LEFT_JUSTIFY)
	...
	def refreshSelection(self):
		...
		if self.iPlayerAddMode == "Units":
			...
			ItemInfo = gc.getPlotEffectInfo(self.iSelection)
			sText = "<font=3>" + CyTranslator().getText("[COLOR_HIGHLIGHT_TEXT]", ()) + ItemInfo.getDescription() + "</color></font>"
			screen.setTableText("WBCurrentItem", 0, 0 , sText, ItemInfo.getButton(), WidgetTypes.WIDGET_PYTHON, 7883, self.iSelection, CvUtil.FONT_LEFT_JUSTIFY)
		else:
...
## Add "," ##
	...
	def handleInput (self, inputClass):
		...
		elif inputClass.getFunctionName() == "AddPlotEffectButton":
			self.iPlayerAddMode = "Effects"
			self.iSelectClass = 0
			self.iSelection = -1
			self.refreshSideMenu()
