...
class WBPlotScreen:
	...
	def placeEffects(self):
		...
		sColor = CyTranslator().getText("[COLOR_POSITIVE_TEXT]", ())
		if iEffect > -1:
			sText = gc.getPlotEffectInfo(iEffect).getDescription()
			sColor = CyTranslator().getText("[COLOR_WARNING_TEXT]", ())
	...
	def handleInput(self, inputClass):
		...
		elif inputClass.getFunctionName() == "WBPlotEffect":
			iEffect = inputClass.getData2()
			if iEditType == 0:
				if bAdd:
					pPlot.setPlotEffectType(iEffect)
				else:
