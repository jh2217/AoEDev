...
class CvDiplomacy:
	...
	def filterUserResponse(self, diploInfo):
			...
			if (self.isUsed(diploInfo.getDiplomacyPowerTypes, i, DiplomacyPowerTypes.NUM_DIPLOMACYPOWER_TYPES)):
				...
				if (ourPower < (theirPower / 2)):
					...
					if not diploInfo.getDiplomacyPowerTypes(i, DiplomacyPowerTypes.DIPLOMACYPOWER_EQUAL):
						continue
			if (not self.isUsed(diploInfo.getLeaderHeadTypes, i, gc.getNumLeaderHeadInfos()) and not (self.isUsed(diploInfo.getCivilizationTypes, i, gc.getNumCivilizationInfos()))):
				for j in range(diploInfo.getNumDiplomacyText(i)):
					genericresponses.append(diploInfo.getDiplomacyText(i, j))
				continue
		...
		if (numResponses>0):
			#rand = gc.getASyncRand()
			iResponse = random.randint(0,numResponses-1)			#test to remove the no diplo crash
			self.setLastResponseID(iResponse)
			return responses[iResponse]
		else:
			if(len(genericresponses)>0):
				iResponse = random.randint(0,len(genericresponses)-1)			#test to remove the no diplo crash
			self.setLastResponseID(iResponse)
			return genericresponses[iResponse]
