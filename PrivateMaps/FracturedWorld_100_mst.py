...
def getGridSize(argsList):
	...
	grid_sizes =	{
						...
						WorldSizeTypes.WORLDSIZE_HUGER:      (51,36)
						}
...
class FracturedWorldTerrainGenerator:
	...
	def __init__(self, iDesertPercent=31, iPlainsPercent=17, iMarshPercent=3,
				 fTundraLatitude=0.72, fTaigaLatitude=0.64,
		...
		fTundraLatitude += self.gc.getClimateInfo(self.map.getClimate()).getSnowLatitudeChange() #must keep getSnow, comes from API
		fTundraLatitude = min(fTundraLatitude, 1.0)
		fTundraLatitude = max(fTundraLatitude, 0.0)
		self.fTundraLatitude = fTundraLatitude

		fTaigaLatitude += self.gc.getClimateInfo(self.map.getClimate()).getTundraLatitudeChange() #must keep getTundra, comes from API
		fTaigaLatitude = min(fTaigaLatitude, 1.0)
		fTaigaLatitude = max(fTaigaLatitude, 0.0)
		self.fTaigaLatitude = fTaigaLatitude
