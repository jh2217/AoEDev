...
def addRivers():
	...
	if not mst.bPfall:
		...
		ringWorld.changeTerrainOrphans( (mst.etTaiga,mst.etGrass,90), (mst.etTaiga,mst.etPlains,90) )
		if mst.bMarsh:
			marshPer = 6 - getAridity()
			mst.mapPrettifier.percentifyTerrain( (mst.etMarsh,marshPer), (mst.etTaiga,2), (mst.etGrass,3) )
	...
	if not mst.bPfall:
		...
		mst.mapPrettifier.lumpifyTerrain( mst.etGrass, mst.etTaiga, mst.etPlains )
		mst.mapPrettifier.lumpifyTerrain( mst.etPlains, mst.etDesert, mst.etGrass )
		# change desert singletons
		ringWorld.changeTerrainOrphans( (mst.etDesert,mst.etPlains,80), (mst.etTundra,mst.etTaiga,99) )
...
def getGridSize(argsList):
	...
	grid_sizes = {	WorldSizeTypes.WORLDSIZE_DUEL:		(  8,4),
						...
						WorldSizeTypes.WORLDSIZE_HUGER:		( 92,4)
	}
		...
		for x in range( iNumPlotsX ):
			for y in range( iNumPlotsY ):
				...
				if i == 8:
					...
					if not mst.bPfall:
						pl.setTerrainType( mst.choose(10,mst.etGrass,mst.etTaiga), True, True )
						if not plot.isPeak():
							plot.setTerrainType( mst.choose(50,mst.etGrass,mst.etTaiga), True, True )
					...
					pl.setPlotType( PlotTypes.PLOT_HILLS, True, True )
					if not mst.bPfall:
						pl.setTerrainType( mst.choose(20,mst.etGrass,mst.etTaiga), True, True )
						if not plot.isPeak():
							plot.setTerrainType( mst.choose(10,mst.etGrass,mst.etTaiga), True, True )
					sprint += "[RingW] Made access for mountain-center at: %r %r \n" % (x,y)
			...
			for y in range(1,15):
				...
				if dxDesert>dxRegion:
					if mst.bPfall:
						...
						if aTerrain[j]==iTundra:
							aTerrain[j] = mst.choose(fChance1,iTaiga,iTundra)
						elif aTerrain[j]==iTaiga:
							aTerrain[j] = mst.choose(fChance1,iGrass,iTaiga)
				...
				if dxGrass>dxRegion:
					if mst.bPfall:
						...
						if aTerrain[j]==iDesert:
							...
							aTerrain[j] = mst.choose(fChance2/2,iTaiga,iGrass)
						elif aTerrain[j]==iTaiga:
							aTerrain[j] = mst.choose(fChance2/2,iTundra,iTaiga)
...
class RingworldTerrainGenerator(CvMapGeneratorUtil.TerrainGenerator):
	def generateTerrain(self):
		...
		for x in range( iNumPlotsX ):
			for y in range( iNumPlotsY ):
				...
				elif terrainData[j]==self.terrainTaiga:
			...
			for y in range( iNumPlotsY ):
				...
				if self.map.plot(x,y).isPeak():
					if y<4 or y>11:
						...
						terrainData[j] = mst.choose( 80, self.terrainTaiga, self.terrainTundra )
					...
					if i > 6:
						...
						terrainData[j] = self.terrainTaiga
	...
	def adjustTerrain(self, x, y, ter, data):
		iChance = 33
		tList = [self.terrainDesert,self.terrainPlains,self.terrainGrass,self.terrainTaiga,self.terrainTundra]
	...
	def generateTerrainAtPlot(self, iX, iY):
		...
		clima_Template = [ self.terrainTundra, self.terrainTaiga, self.terrainGrass, self.terrainPlains, self.terrainDesert ]
		...
		# Arid Climate: Tundra<-Taiga<-Grass, Grass->Plains->Desert
		if (sClimate == "Arid") and mst.choose( 10, True, False ):
			if terrainVal==self.terrainPlains    and mst.choose( 90, True, False ):
					...
					terrainVal = self.terrainTaiga		# 0.5%
			elif terrainVal==self.terrainTaiga  and mst.choose( 10, True, False ):
					terrainVal = self.terrainTundra			# 1%
		# Tropical Climate: Tundra->Taiga->Grass, Grass<-Plains<-Desert
			...
			elif terrainVal==self.terrainTaiga  and mst.choose( 30, True, False ):
					terrainVal = self.terrainGrass		# 3%
			elif terrainVal==self.terrainTundra     and mst.choose( 40, True, False ):
					terrainVal = self.terrainTaiga		# 4%
		# Cold Climate: Tundra<-Taiga<-Grass<-Plains<-Desert
		if (sClimate == "Cold") and mst.choose( 10, True, False ):
			if terrainVal==self.terrainDesert    and mst.choose( 50, True, False ):
					...
					terrainVal = self.terrainTaiga		# 0.5%
			elif terrainVal==self.terrainTaiga  and mst.choose( 20, True, False ):
					terrainVal = self.terrainTundra			# 2%
