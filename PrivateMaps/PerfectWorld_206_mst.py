		...
		for y in range(mc.height):
			for x in range(mc.width):
				...
				#based on how close to the threshold the rainfall is
					...
					if self.averageTempMap[i] < mc.TundraTemp:
		...
		for y in range(mc.height):
			for x in range(mc.width):
				...
				if self.terrainMap[i] == mc.OCEAN or self.terrainMap[i] == mc.COAST:
					continue
				if self.terrainMap[i] == mc.TUNDRA:
					lowerFound = False
					for direction in range(1,9,1):
						xx,yy = GetXYFromDirection(x,y,direction)
						ii = GetIndex(xx,yy)
						...
						and self.terrainMap[ii] != mc.TUNDRA:
							lowerFound = True
						if self.terrainMap[ii] != mc.TAIGA and self.terrainMap[ii] != mc.TUNDRA and\
					...
					if lowerFound and self.plotMap[i] == mc.LAND:
						self.plotMap[i] = mc.HILLS
				elif self.terrainMap[i] == mc.TAIGA:
					lowerFound = False
					for direction in range(1,9,1):
						xx,yy = GetXYFromDirection(x,y,direction)
						ii = GetIndex(xx,yy)
						if ii == -1:
							continue
						if self.terrainMap[ii] != mc.OCEAN and self.terrainMap[ii] != mc.COAST \
						and self.terrainMap[ii] != mc.TUNDRA and self.terrainMap[ii] != mc.TAIGA:
							lowerFound = True
						if self.terrainMap[ii] == mc.DESERT:
							self.terrainMap[i] = mc.PLAINS
					if lowerFound and self.plotMap[i] == mc.LAND:
					...
					for direction in range(1,9,1):
						xx,yy = GetXYFromDirection(x,y,direction)
						ii = GetIndex(xx,yy)
						if self.terrainMap[ii] == mc.TUNDRA or self.terrainMap[ii] == mc.TAIGA:
							higherFound = True
...
def getGridSize(argsList):
	...
	grid_sizes =	{
						...
						WorldSizeTypes.WORLDSIZE_HUGER:		(48,36)
						}
...
##### Temudjin START
	if mst.bPfall:
		...
		terrainList = [0, 1, 5, 4, 3, 2, 6, 7]		# Ocean, Coast, Desert, Plains, Grass, Marsh, Taiga, Tundra
