...
bool CvDeal::startTrade(TradeData trade, PlayerTypes eFromPlayer, PlayerTypes eToPlayer)
{
	...
	case TRADE_CIVIC:
		...
		FAssertMsg(GC.getCivicInfo((CivicTypes)trade.m_iData).getCivicOptionType() >= 0, "Warning, About to Leak Memory in CvDeal::startTrade");
	...
	kTrigger.m_iTrade = trade.m_eItemType;
	GET_PLAYER(eFromPlayer).doTraitTriggers(TRAITHOOK_TRADE, &kTrigger);
	// DynTraits End
	return bSave;
}
