...
CyCity* CyMap::findCity(int iX, int iY, int /*PlayerTypes*/ eOwner, int /*TeamTypes*/ eTeam, bool bSameArea, bool bCoastalOnly, int /*TeamTypes*/ eTeamAtWarWith, int /*DirectionTypes*/ eDirection, CyCity* pSkipCity, bool bIgnoreSettlements)
{
	return m_pMap ? new CyCity(m_pMap->findCity(iX, iY, (PlayerTypes)eOwner, (TeamTypes)eTeam, bSameArea, bCoastalOnly, ((TeamTypes)eTeamAtWarWith), (DirectionTypes)eDirection, pSkipCity->getCity(), bIgnoreSettlements)) : NULL;
}
...
/*************************************************************************************************/
bool CyMap::isNeedsRebuilding()
{
	return m_pMap ? m_pMap->isNeedsRebuilding() : false;
}

void CyMap::setNeedsRebuilding(bool bNewValue)
{
	if (m_pMap)
	{
		m_pMap->setNeedsRebuilding(bNewValue);
	}
}

void CyMap::rebuildGraphics()
{
	if (m_pMap)
	{
		m_pMap->rebuildGraphics();
	}
}
/*************************************************************************************************/
/**	FastRebuild								END													**/
