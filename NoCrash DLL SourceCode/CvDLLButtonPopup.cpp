...
/**																								**/
/**					Radius is handled in the setting of settlement or not now					**/
/*************************************************************************************************/
/**								---- Start Original Code ----									**
				pCity->setPlotRadius(3);
/**								----  End Original Code  ----									**/
				pCity->setSettlement(false);
				CvMessageControl::getInstance().sendModNetMessage(21, pCity->getID(), 0, pCity->getOwner(), -1);
...
/*************************************************************************************************/
		...
		if (pPopupReturn->getButtonClicked() == 1)
		{
			...
			if (NULL != pCity)
			{
				...
				CvMessageControl::getInstance().sendModNetMessage(21, pCity->getID(), 1, pCity->getOwner(), -1);
			}
		}
		break;
	case BUTTONPOPUP_GOBLINCITYCLASS:
		if (pPopupReturn->getButtonClicked() == 0)
		{
			CvCity* pCity = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCity(info.getData1());
			if (NULL != pCity)
			{
				...
				/**								---- Start Original Code ----									**
								pCity->setPlotRadius(3);
				/**								----  End Original Code  ----									**/
				pCity->setCityClass((CityClassTypes)GC.getInfoTypeForString("CITYCLASS_SCORPION"));
				CvMessageControl::getInstance().sendModNetMessage(20, pCity->getID(), GC.getInfoTypeForString("CITYCLASS_SCORPION"), pCity->getOwner(),-1);
				/*************************************************************************************************/
				/**	Tweak									END													**/
				/*************************************************************************************************/
			}
		...
		if (pPopupReturn->getButtonClicked() == 1)
		{
			...
			if (NULL != pCity)
			{
				pCity->setCityClass((CityClassTypes)GC.getInfoTypeForString("CITYCLASS_NEITH"));
				CvMessageControl::getInstance().sendModNetMessage(20, pCity->getID(), GC.getInfoTypeForString("CITYCLASS_NEITH"), pCity->getOwner(), -1);
			}
		}
		if (pPopupReturn->getButtonClicked() == 2)
		{
			...
			if (NULL != pCity)
			{
				pCity->setCityClass((CityClassTypes)GC.getInfoTypeForString("CITYCLASS_MURIS"));
				CvMessageControl::getInstance().sendModNetMessage(20, pCity->getID(), GC.getInfoTypeForString("CITYCLASS_MURIS"), pCity->getOwner(), -1);
		...
		if (pPopupReturn->getButtonClicked() == 3)
		{
			...
			if (NULL != pCity)
			{
				pCity->setCityClass((CityClassTypes)GC.getInfoTypeForString("CITYCLASS_LUKOS"));
				CvMessageControl::getInstance().sendModNetMessage(20, pCity->getID(), GC.getInfoTypeForString("CITYCLASS_LUKOS"), pCity->getOwner(), -1);


			}
		}
		break;
	case BUTTONPOPUP_TRAITCHOICE:
		if (pPopupReturn->getButtonClicked() == 0)
		{
			CvPlayer& pPlayer = GET_PLAYER(GC.getGameINLINE().getActivePlayer());
			pPlayer.setHasTrait((TraitTypes)info.getData2(), true);
			pPlayer.initValidTraitTriggers(false);
			pPlayer.setGainingTrait(false);
			CvMessageControl::getInstance().sendModNetMessage(22, info.getData2(), 1, GC.getGameINLINE().getActivePlayer(), -1);

		}
		else
		{
			CvPlayer& pPlayer = GET_PLAYER(GC.getGameINLINE().getActivePlayer());
			pPlayer.setTraitPoints((TraitTypes)info.getData2(), 0);
			pPlayer.setGainingTrait(false);
			CvMessageControl::getInstance().sendModNetMessage(22, info.getData2(), 0, GC.getGameINLINE().getActivePlayer(), -1);
...
//FfH: End Add
	...
	case BUTTONPOPUP_EXTENDED_GAME:
		...
		else if (pPopupReturn->getButtonClicked() == 2) // test Ronkhar to propose reloading when the player just lost
		{	// load game
			GC.getGameINLINE().doControl(CONTROL_LOAD_GAME);
		}
		break;
...
//FfH: Added by Kael 11/04/2007
	...
	case BUTTONPOPUP_CONFIRMSETTLEMENT:
		...
		break;
	case BUTTONPOPUP_GOBLINCITYCLASS:
		bLaunched = launchChooseGoblinClassPopup(pPopup, info);
		break;
	case BUTTONPOPUP_TRAITCHOICE:
...
//InterfaceUpgrade: Better Popups - Added by Grey Fox 04/17/2008
	gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
	gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/separator_recommended.dds", 340, 12);
//InterfaceUpgrade: Better Popups - End Add
...
//FfH: Modified by Kael 02/06/2009
//		gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, szBuffer, GET_PLAYER(pCity->getOwnerINLINE()).getUnitButton(eProductionUnit), GC.getUnitInfo(eProductionUnit).getUnitClassType(), WIDGET_TRAIN, GC.getUnitInfo(eProductionUnit).getUnitClassType(), pCity->getID(), true, POPUP_LAYOUT_STRETCH, DLL_FONT_LEFT_JUSTIFY );
		szButton = GET_PLAYER(pCity->getOwnerINLINE()).getUnitButton(eProductionUnit);
		if (GC.getCivilizationInfo(pCity->getCivilizationType()).getDefaultRace() != NO_PROMOTION)
		{
			szButton = GC.getUnitInfo(eProductionUnit).getArtInfo(0, NO_ERA, (UnitArtStyleTypes)GC.getPromotionInfo((PromotionTypes)GC.getCivilizationInfo(pCity->getCivilizationType()).getDefaultRace()).getUnitArtStyleType())->getButton();
		}
...
//InterfaceUpgrade: Better Popups - Modified by Grey Fox 04/17/2008
	if (!GC.getDefineINT("USE_BETTER_POPUPS"))
	{
	gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
	gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/separator_recommended.dds", 340, 12);
//InterfaceUpgrade: Better Popups - End Modify
	for (int iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
	{
		//UnitTypes eLoopUnit = (UnitTypes)GC.getCivilizationInfo(pCity->getCivilizationType()).getCivilizationUnits(iI);
		UnitTypes eLoopUnit = (UnitTypes)pCity->getCityUnits(iI);
...
//FfH: Modified by Kael 02/04/2009
//					gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, szBuffer, GET_PLAYER(pCity->getOwnerINLINE()).getUnitButton(eLoopUnit), iI, WIDGET_TRAIN, iI, pCity->getID(), true, POPUP_LAYOUT_STRETCH, DLL_FONT_LEFT_JUSTIFY );
					szButton = GET_PLAYER(pCity->getOwnerINLINE()).getUnitButton(eLoopUnit);
					if (GC.getCivilizationInfo(pCity->getCivilizationType()).getDefaultRace() != NO_PROMOTION)
					{
						szButton = GC.getUnitInfo(eLoopUnit).getArtInfo(0, NO_ERA, (UnitArtStyleTypes)GC.getPromotionInfo((PromotionTypes)GC.getCivilizationInfo(pCity->getCivilizationType()).getDefaultRace()).getUnitArtStyleType())->getButton();
					}
...
//InterfaceUpgrade: Better Popups - Modified by Grey Fox 04/17/2008
	...
	{
		int iNum = 0;
		int* iNumCombats = new int[GC.getNumUnitCombatInfos()];
			...
			if (eLoopUnit != NO_UNIT)
			{
				if (eLoopUnit != eProductionUnit)
				{
					if (pCity->canTrain(eLoopUnit))
					{
						...
						for (int iJ = 0; iJ < GC.getNumUnitCombatInfos(); iJ++)
						{
							if (GC.getUnitInfo(eLoopUnit).getUnitCombatType() == (UnitCombatTypes)iJ)
							{
								iNumCombats[iJ]++;
							}
						}
		...
		if (iNum > 0)
		{
			gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
			gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
			gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/separator_units.dds", 340, 12);

			//***************************************************************
			// I tried to do this a smother way, but this works.
			for (int iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
			{
				//UnitTypes eLoopUnit = (UnitTypes)GC.getCivilizationInfo(pCity->getCivilizationType()).getCivilizationUnits(iI);
				UnitTypes eLoopUnit = (UnitTypes)pCity->getCityUnits(iI);
				if (eLoopUnit != NO_UNIT)
				{
					if (eLoopUnit != eProductionUnit)
					{
						if (pCity->canTrain(eLoopUnit))
						{
							if (GC.getUnitInfo(eLoopUnit).getUnitCombatType() == NO_UNITCOMBAT) //First we list all units with no unitcombat
							{
								int iTurns = pCity->getProductionTurnsLeft(eLoopUnit, 0);
								szBuffer.Format(L"%s (%d)", GC.getUnitInfo(eLoopUnit).getDescription(), iTurns);
								gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, szBuffer, GET_PLAYER(pCity->getOwnerINLINE()).getUnitButton(eLoopUnit), iI, WIDGET_TRAIN, iI, pCity->getID(), true, POPUP_LAYOUT_STRETCH, DLL_FONT_LEFT_JUSTIFY );
			...
			for (int iJ = 0; iJ < GC.getNumUnitCombatInfos(); iJ++)
			{
				if (iNumCombats[iJ] > 0)
				{
					switch(iJ)
					{
						case 0: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/recon_units.dds", 340, 12); break;
						case 1: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/archer_units.dds", 340, 12); break;
						case 2: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/mounted_units.dds", 340, 12); break;
						case 3: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/melee_units.dds", 340, 12); break;
						case 4: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/siege_units.dds", 340, 12); break;
						case 5: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/arcane_units.dds", 340, 12); break;
						case 6: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/disciple_units.dds", 340, 12); break;
				...
				for (int iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
				{
					...
					if (eLoopUnit != NO_UNIT)
					{
						if (GC.getUnitInfo(eLoopUnit).getUnitClassType() != iI)
						{
							continue;
						}
						if (eLoopUnit != eProductionUnit)
...
//InterfaceUpgrade: Better Popups - End Modify
	...
	{
		BuildingTypes eLoopBuilding = ((BuildingTypes)(pCity->getCityBuildings(iI)));
...
//InterfaceUpgrade: Better Popups - Modified by Grey Fox 04/17/2008
	...
	{
		int iNum = 0;
		int* iNumFlavors = new int[GC.getNumFlavorTypes()];
			...
			if (eLoopBuilding != NO_BUILDING)
			{
				if (eLoopBuilding != eProductionBuilding)
				{
					if (pCity->canConstruct(eLoopBuilding))
					{
						...
						for (int iJ = 0; iJ < GC.getNumFlavorTypes(); iJ++)
						{
							if (GC.getBuildingInfo(eLoopBuilding).getFlavorValue((FlavorTypes)iJ) >= 5)
							{
								iNumFlavors[iJ]++;
							}
						}
		...
		if (iNum > 0)
		{
			...
			for (int iJ = 0; iJ < GC.getNumFlavorTypes(); iJ++)
			{
				if (iNumFlavors[iJ] > 0)
				{
					switch(iJ)
					{
						case 0: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/military_buildings.dds", 340, 12); break;
						case 1: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/religion_buildings.dds", 340, 12); break;
						case 2: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/industry_buildings.dds", 340, 12); break;
						case 3: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/economy_buildings.dds", 340, 12); break;
						case 4: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/science_buildings.dds", 340, 12); break;
						case 5: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/culture_buildings.dds", 340, 12); break;
						case 6: gDLL->getInterfaceIFace()->popupAddSeparator(pPopup); gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/growth_buildings.dds", 340, 12); break;
					}
				}
				else
				{
							...
							if (pCity->canConstruct(eLoopBuilding))
							{
								if (GC.getBuildingInfo(eLoopBuilding).getFlavorValue((FlavorTypes)iJ) >= 5)
								{
									...
									for (int iK = 0; iK < GC.getNumFlavorTypes(); iK++)
									{
										if (iK != iJ)
										{
											if (GC.getBuildingInfo(eLoopBuilding).getFlavorValue((FlavorTypes)iK) >= GC.getBuildingInfo(eLoopBuilding).getFlavorValue((FlavorTypes)iJ))
											{
												bValid = false;
											}
										}
	...
	if (GC.getDefineINT("USE_BETTER_POPUPS"))
	{
		...
		if (iNum > 0)
		{
			gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
			gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
			gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/separator_rituals.dds", 340, 12);
	for (int iI = 0; iI < GC.getNumProjectInfos(); iI++)
	{
		if (iI != eProductionProject)
		{
...
//InterfaceUpgrade: Better Popups - Modified by Grey Fox 04/17/2008
	...
	if (GC.getDefineINT("USE_BETTER_POPUPS"))
	{
		...
		if (iNum > 0)
		{
			gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
			gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
			gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/Interface/Popup/separator_processes.dds", 340, 12);
	for (int iI = 0; iI < GC.getNumProcessInfos(); iI++)
	{
		if (iI != eProductionProcess)
		{
...
//Goblin cityclass choice, hardcoded for now, need to be better
bool CvDLLButtonPopup::launchChooseGoblinClassPopup(CvPopup* pPopup, CvPopupInfo& info)
{
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_GOBLIN_CLASS_CHOICE");
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_SCORPION").c_str(), NULL, 0);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NEITH").c_str(), NULL, 1);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_MURIS").c_str(), NULL, 2);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_LUKOS").c_str(), NULL, 3);
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);
...
{
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_TRAIT_GAIN_CHOICE",GC.getTraitInfo((TraitTypes)info.getData2()).getDescription());
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_YES").c_str(), NULL, 0,WIDGET_HELP_TRAIT, info.getData2(),GET_PLAYER((PlayerTypes)info.getData1()).getCivilizationType());
...
//FfH: Modified by Kael 09/25/2007 (to remove the regenerate map option)
...
bool CvDLLButtonPopup::launchPythonPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	{
		if (info.getOption2() && !info.getPythonButtonArt(i).IsEmpty())
		{
			gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, info.getPythonButtonText(i), NULL, i, (WidgetTypes)info.getFlags(), GC.getInfoTypeForString(info.getPythonButtonArt(i)), GC.getGameINLINE().getActivePlayer());
		}
		else
		{
...
bool CvDLLButtonPopup::launchExtendedGamePopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_LOAD_GAME").c_str(), NULL, 2, WIDGET_GENERAL, 2, 0, true, POPUP_LAYOUT_STRETCH, DLL_FONT_CENTER_JUSTIFY); // test Ronkhar
...
bool CvDLLButtonPopup::launchEventPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	for (int i = 0; i < kTrigger.getNumEvents(); i++)
	{
		...
		else if (kActivePlayer.isOption(PLAYEROPTION_EVENTS_SHOW_BLOCKED_ANSWERS))
		{
