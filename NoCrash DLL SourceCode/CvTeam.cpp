...
/*************************************************************************************************/
	if (eID == ORC_TEAM)
	{
		GET_TEAM(ANIMAL_TEAM).init(ANIMAL_TEAM);
		GET_TEAM(DEMON_TEAM).init(DEMON_TEAM);
	}
/*************************************************************************************************/
/**	MultiBarb								END													**/
/*************************************************************************************************/
...
/*************************************************************************************************/
	...
	if (!bConstructorCall)
	{
		...
		for (iI = 0; iI < GC.getNumProjectInfos(); iI++)
		{
			...
			for (iJ = 0; iJ < GC.getMaxNumProjectsAllowed(); iJ++)
			{
...
/*************************************************************************************************/
	if (!bConstructorCall)
	{
		if (eID == ORC_TEAM)
		{
			GET_TEAM(ANIMAL_TEAM).reset(ANIMAL_TEAM, bConstructorCall);
			GET_TEAM(DEMON_TEAM).reset(DEMON_TEAM, bConstructorCall);
		}
	}
/*************************************************************************************************/
/**	MultiBarb								END													**/
	...
	if (getID() == ORC_TEAM || getID() == DEMON_TEAM)
	{
		for (iI = 0; iI < GC.getNumTechInfos(); iI++)
		{
			if (!isHasTech((TechTypes)iI))
			{
				...
				if (iCount > 0)
				{
					// Orcs gain FREE_TECH_PERCENT (50) of total cost per turn, multiplied by % of civs that know the tech, modulated by gamespeed.
					// E.g. 3 of 9 civs know a tech, so orcs get 0.5 * 0.33 * (gamespeed modifier) of the tech each turn. [100 / FREE_TECH * # of civs] is the max turns of tech lag.
					if (getID() == ORC_TEAM)
					{
					...
					// Demons get a flat percent (per difficulty) for each team that knows; from 1% settler to 15% deity
...
/*************************************************************************************************/
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
			{
				if (((GET_PLAYER((PlayerTypes)iI).getTeam() == getID()) || (GET_PLAYER((PlayerTypes)iI).getTeam() == eTeam)) && (GET_PLAYER((PlayerTypes)iI).getNumCities()))
				{
...
/*************************************************************************************************/
/**	Cultured Trait 	 	Orbis from Sanguo Mod		05/03/09	Ahwaric		**/
/*************************************************************************************************/
		int iOurWarCount = getAtWarCount(true);
		int iTheirWarCount = GET_TEAM(eTeam).getAtWarCount(true);
		if ((iOurWarCount == 1 || iTheirWarCount == 1) && !isBarbarian() && !isMinorCiv() && !GET_TEAM(eTeam).isBarbarian() && !GET_TEAM(eTeam).isMinorCiv())
		{
			for (iI = 0; iI < MAX_PLAYERS; iI++)
			{
							...
							for (int iJ = 0; iJ < GC.getNumTraitInfos(); iJ++)
							{
								if (kPlayer.hasTrait((TraitTypes)iJ))
								{
									for (int iK = 0; iK < NUM_COMMERCE_TYPES; iK++)
									{
										...
										{
											kPlayer.changeCommerceRateModifier(((CommerceTypes)iK), GC.getTraitInfo((TraitTypes)iJ).getPeaceCommerceModifier(iK) * (-1));
										}
									}
								}
							}
...
/************************************************************************************************/
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).isAlive() && GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
			{
				...
				GET_PLAYER((PlayerTypes)iI).doTraitTriggers(TRAITHOOK_START_WAR, &kTrigger);
			}
		}
		// DynTraits End
...
/**																								**/
/**				Allows Announcing of War/Peace changes with Barbarian Civilizations				**/
/*************************************************************************************************/
/**								---- Start Original Code ----									**
...
/*************************************************************************************************/
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
			{
				if (((GET_PLAYER((PlayerTypes)iI).getTeam() == getID()) || (GET_PLAYER((PlayerTypes)iI).getTeam() == eTeam))&& (GET_PLAYER((PlayerTypes)iI).getNumCities()>0))
				{
					GET_PLAYER((PlayerTypes)iI).updatePlotGroups();
				}
			}
...
/************************************************************************************************/
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			...
			{
				TraitTriggeredData kTrigger;
				GET_PLAYER((PlayerTypes)iI).doTraitTriggers(TRAITHOOK_MAKE_PEACE, &kTrigger);
			}
		}
		// DynTraits End
...
/*************************************************************************************************/
/**	GreyFoxMod							01/05/09									Xienwolf	**/
/**																								**/
/**						Modifies Tech Costs based on Current Tech & Bonuses						**/
/*************************************************************************************************/
	if (GC.getTechInfo(eTech).getNumTechCostShifts() > 0)
	{
		for (int iI = 0; iI < GC.getTechInfo(eTech).getNumTechCostShifts(); iI++)
		{

			if (isHasTech((TechTypes)GC.getTechInfo(eTech).getTechCostShift(iI)))
			{
				iCost += GC.getTechInfo(eTech).getTechCostShift(iI,true);
	...
	for (int iI = 0; iI < GC.getNumBonusInfos(); iI++)
	{
		...
		{
			if (GC.getTechInfo(eTech).getBonusCostShift((BonusTypes)iI) != 0)
			{
				iCost += GC.getTechInfo(eTech).getBonusCostShift((BonusTypes)iI);
			}
		}
	}
	if (GC.getTechInfo(eTech).getNumTechCostMods() > 0)
	{
		for (int iI = 0; iI < GC.getTechInfo(eTech).getNumTechCostMods(); iI++)
		{

			if (isHasTech((TechTypes)GC.getTechInfo(eTech).getTechCostMod(iI)))
			{
				iCost *= GC.getTechInfo(eTech).getTechCostMod(iI,true);
				iCost /= 100;
	...
	for (int iI = 0; iI < GC.getNumBonusInfos(); iI++)
	{
		if (hasBonus((BonusTypes)iI))
		{
			if (GC.getTechInfo(eTech).getBonusCostMod((BonusTypes)iI) != 0)
			{
				iCost *= GC.getTechInfo(eTech).getBonusCostMod((BonusTypes)iI);
				iCost /= 100;
			}
		}
	}
...
/**																								**/
/**			Having this function may make a few of the other calls for this obsolete			**/
/*************************************************************************************************/
	if (getBlockBonuses() > 0)
	{
		return true;
	}
/*************************************************************************************************/
...
/**								----  End Original Code  ----									**/
	return (getID() == ORC_TEAM || getID() == ANIMAL_TEAM || getID() == DEMON_TEAM);
/*************************************************************************************************/
/**	Tweak									END													**/
/*************************************************************************************************/
}
...
void CvTeam::changeExtraWaterSeeFromCount(int iChange)
{
	if (iChange != 0)
	{
		GC.getMapINLINE().updateSight(false,false);
...
/**								----  End Original Code  ----									**/
				if (!isBarbarian() && (eIndex != ORC_TEAM && eIndex != DEMON_TEAM && eIndex != ANIMAL_TEAM))
				{
					declareWar(eIndex, false, NO_WARPLAN);
				}
/*************************************************************************************************/
...
void CvTeam::setVassal(TeamTypes eIndex, bool bNewValue, bool bCapitulated)
{
	...
	if (isVassal(eIndex) != bNewValue)
	{
		...
		for (int i = 0; i < MAX_PLAYERS; i++)
		{
			...
			{
				TraitTriggeredData kTrigger;
				TraitHookTypes eHook = bNewValue ? TRAITHOOK_MAKE_VASSAL : TRAITHOOK_LOSE_VASSAL;
				GET_PLAYER((PlayerTypes)i).doTraitTriggers(eHook, &kTrigger);
			}
			else if (GET_PLAYER((PlayerTypes)i).getTeam() == eIndex)
			{
				TraitTriggeredData kTrigger;
				TraitHookTypes eHook = bNewValue ? TRAITHOOK_BECOME_VASSAL : TRAITHOOK_FREED_VASSAL;
				GET_PLAYER((PlayerTypes)i).doTraitTriggers(eHook, &kTrigger);
			}
		}
		// DynTraits End
...
/**	Xienwolf Tweak							06/18/09											**/
/**																								**/
/**							Prevents double correction for Summoned units						**/
/*************************************************************************************************/
/**								---- Start Original Code ----									**
...
/**																								**/
/**							Remove Colony status from the former Vassal.						**/
/*************************************************************************************************/
				setColony(false);
/*************************************************************************************************/
/**	StateNames								END													**/
...
/**																								**/
/**		Not applicable (intended to work this way). Also, what? who? why? (spaceship?)			**/
/*************************************************************************************************/
/**								---- Start Original Code ----									**
...
/*************************************************************************************************/
	if (kProject.getCooldown() > -1)
	{
		if (kProject.getMaxTeamInstances() > -1)
		{
			setProjectTimer(eIndex);
		}
		if (kProject.getMaxGlobalInstances() > -1)
		{
			GC.getGameINLINE().setProjectTimer(eIndex);
		}
	}
/*************************************************************************************************/
/**	New Tag Defs							END													**/
...
/*************************************************************************************************/
								...
								if (eBestPlayer != NO_PLAYER)
								{
									...
									else
									{
										...
										for (iJ = 0; iJ < MAX_PLAYERS; iJ++) // test Ronkhar to transmit religion to all team members
										{
											if (GET_PLAYER((PlayerTypes)iJ).isAlive() && !(GET_PLAYER((PlayerTypes)iJ).getModReligionSpreadChance() < -100))
											{
												if (GET_PLAYER((PlayerTypes)iJ).getTeam() == getID())
												{
													if (((PlayerTypes)iJ)!= eBestPlayer)
													{
														pCapitalCity = GET_PLAYER((PlayerTypes)iJ).getCapitalCity();
														if (pCapitalCity != NULL)
														{
															pCapitalCity->setHasReligion((ReligionTypes)iI, true, true, true);
														}
													}
												}
											}
...
/**																								**/
/**						Extra Alignment Shift due to Discovering a Tech							**/
/*************************************************************************************************/
					if (GC.getGameINLINE().isOption(GAMEOPTION_BROADER_ALIGNMENTS))
					{
						if (GC.getTechInfo(eIndex).getFirstToTechAlignmentModifier() != 0)
						{
							GET_PLAYER(ePlayer).changeBroadEventModifier(GC.getTechInfo(eIndex).getFirstToTechAlignmentModifier());
							GET_PLAYER(ePlayer).updateAlignment();
						}
/*************************************************************************************************/
...
/*************************************************************************************************/
						if (GC.getTechInfo(eIndex).getFirstToTechEthicalAlignmentModifier() != 0)
						{
							GET_PLAYER(ePlayer).changeBroadEthicalEventModifier(GC.getTechInfo(eIndex).getFirstToTechEthicalAlignmentModifier());
							GET_PLAYER(ePlayer).updateEthicalAlignment();
						}
/*************************************************************************************************/
...
/************************************************************************************************/
		...
		if (GC.getTechInfo(eIndex).isInfluenceAllowed())
		{
			GET_PLAYER(ePlayer).changeInfluenceAllowed(1 * iChange);
		}
		if (GC.getTechInfo(eIndex).getVictoryInfluenceModifier() != 100)
		{
			GET_PLAYER(ePlayer).changeVictoryInfluenceModifier((GC.getTechInfo(eIndex).getVictoryInfluenceModifier() - 100) * iChange);
		}
		if (GC.getTechInfo(eIndex).getDefeatInfluenceModifier() != 100)
		{
			GET_PLAYER(ePlayer).changeDefeatInfluenceModifier((GC.getTechInfo(eIndex).getDefeatInfluenceModifier() - 100) * iChange);
		}
		if (GC.getTechInfo(eIndex).getPillageInfluenceModifier() != 100)
		{
			GET_PLAYER(ePlayer).changePillageInfluenceModifier((GC.getTechInfo(eIndex).getPillageInfluenceModifier() - 100) * iChange);
		}
/*************************************************************************************************/
/**	END																							**/
/*************************************************************************************************/
...
void CvTeam::processTech(TechTypes eTech, int iChange)
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
		{
			...
			kData.m_iTech = eTech;
			kData.m_bFirst = (GC.getGameINLINE().countKnownTechNumTeams(eTech) == 1);
			GET_PLAYER((PlayerTypes)iI).doTraitTriggers(TRAITHOOK_TECH, &kData);
/*************************************************************************************************/
/**	Broader Alignments						04/08/08	Written: Grey Fox	Imported: Xienwolf	**/
/**																								**/
/**						Modify Alignment based on Techs Researched								**/
/*************************************************************************************************/
			if (GC.getGameINLINE().isOption(GAMEOPTION_BROADER_ALIGNMENTS))
			{
				if (GC.getTechInfo(eTech).getAlignmentModifier() != 0)
				{
					GET_PLAYER((PlayerTypes)iI).changeBroadEventModifier(GC.getTechInfo(eTech).getAlignmentModifier() * iChange);
					GET_PLAYER((PlayerTypes)iI).updateAlignment();
				}
/*************************************************************************************************/
/**	Lawful-Chaotic Alignments 				11/06/09								Valkrionn	**/
/**																								**/
/**							Adds a new alignment axis to the game								**/
/*************************************************************************************************/
				if (GC.getTechInfo(eTech).getEthicalAlignmentModifier() != 0)
				{
					GET_PLAYER((PlayerTypes)iI).changeBroadEthicalEventModifier(GC.getTechInfo(eTech).getEthicalAlignmentModifier() * iChange);
...
/*************************************************************************************************/
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
		{
			if (GET_PLAYER((PlayerTypes)iI).getInitialCityCap() > 0 && GET_PLAYER((PlayerTypes)iI).getMaxCityCap() == 0)
			{
				GET_PLAYER((PlayerTypes)iI).changePopulationCap(GC.getTechInfo(eTech).getCityCapModifier() * iChange);
			}

			if (GET_PLAYER((PlayerTypes)iI).getInitialCityCap() > 0 && GET_PLAYER((PlayerTypes)iI).getMaxCityCap() > 0)
			{
				if (GET_PLAYER((PlayerTypes)iI).getPopulationCap() < GET_PLAYER((PlayerTypes)iI).getMaxCityCap())
				{
					GET_PLAYER((PlayerTypes)iI).changePopulationCap(GC.getTechInfo(eTech).getCityCapModifier() * iChange);
				}
			}
...
/*************************************************************************************************/
			GET_PLAYER((PlayerTypes)iI).updateRouteCache();
/*************************************************************************************************/
/**	Speedup									END													**/
/*************************************************************************************************/
		}
...
/*************************************************************************************************/
bool CvTeam::canEverResearch(TechTypes eTech) const
{
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
			{
				if (GET_PLAYER((PlayerTypes)iI).canEverResearch(eTech))
				{
					return true;
				}
			}
...
int CvTeam::getBlockBonuses() const
{
	return m_iBlockBonuses;
}
void CvTeam::changeBlockBonuses(int iChange)
{
	CvPlot* pLoopPlot;
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
	...
	m_iBlockBonuses = std::max(0, m_iBlockBonuses + iChange);
...
int CvTeam::getRevealAllBonuses()
{
	return m_iRevealAllBonuses;
}
void CvTeam::changeRevealAllBonuses(int iChange)
{
	if ((std::max(0,m_iRevealAllBonuses + iChange) == 0) != (m_iRevealAllBonuses == 0))
	{
		CvPlot* pLoopPlot;
		bool bRevealed = (getRevealAllBonuses() > 0);
		for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
		{
			pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
			if (pLoopPlot->getBonusType() != NO_BONUS)
		...
		m_iRevealAllBonuses = std::max(0, m_iRevealAllBonuses + iChange);
		...
		if (bRevealed != (getRevealAllBonuses() > 0))
		{
			GC.getMapINLINE().rebuildGraphics();
		}
	}
	else
	{
...
int CvTeam::getHideUnits()
{
	return m_iHideUnits;
}
void CvTeam::changeHideUnits(int iChange)
{
	bool bInitial = (getHideUnits() > 0);
	m_iHideUnits = std::max(0, m_iHideUnits + iChange);
	if ((getHideUnits() > 0) != bInitial)
	{
		for (int iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
			{
				if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
				{
					GET_PLAYER((PlayerTypes)iI).updateShroudedUnits();
				}
			}
		}
	}
...
int CvTeam::getSeeInvisible()
{
	return m_iSeeInvisible;
}
void CvTeam::changeSeeInvisible(int iChange)
{
	m_iSeeInvisible = std::max(0, m_iSeeInvisible + iChange);
}

int CvTeam::getProjectTimer(ProjectTypes eProject, int iCount)
{
	return m_ppaaiProjectTimers[eProject][iCount];
}
void CvTeam::clearProjectTimer(ProjectTypes eProject)
{
	...
	RestoreProjectCount(eProject);
}
void CvTeam::setProjectTimer(ProjectTypes eProject)
{
...
void CvTeam::RestoreProjectCount(ProjectTypes eProject)
{
/*************************************************************************************************/
/**	Tweak							10/05/10								Snarko				**/
/**																								**/
/**					Bugfix, we shalln't set this to zero OR LESS!								**/
/*************************************************************************************************/
/**								---- Start Original Code ----									**
	m_paiProjectCount[eProject] = std::min(0, (m_paiProjectCount[eProject] - 1));
/**								----  End Original Code  ----									**/
	m_paiProjectCount[eProject] = std::max(0, (m_paiProjectCount[eProject] - 1));
/*************************************************************************************************/
/**	Tweak								END														**/
/*************************************************************************************************/
}

bool CvTeam::isColony() const
{
	return m_bColony;
}

void CvTeam::setColony(bool bNewValue)
{
	if (isColony() != bNewValue)
	{
		m_bColony = bNewValue;
	}
}
...
/*************************************************************************************************/
	pStream->Read(&m_iBlockBonuses);
	pStream->Read(&m_iRevealAllBonuses);
...
/*************************************************************************************************/
	pStream->Write(m_iBlockBonuses);
	pStream->Write(m_iRevealAllBonuses);
...
bool CvTeam::isRevealBonus(BonusTypes eBonus) const
{
	for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		...
		if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
		{
			if (GET_PLAYER((PlayerTypes)iI).isRevealBonus(eBonus))
			{
				return true;
			}
		}
