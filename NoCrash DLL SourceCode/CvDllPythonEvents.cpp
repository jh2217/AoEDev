...
void CvDllPythonEvents::reportTraitGained(TraitTypes eTrait, PlayerTypes ePlayer)
{
	if (preEvent())
	{
		CyArgsList eventData;
		eventData.add("traitGained");						// add key to lookup python handler fxn

		eventData.add((int)eTrait);
		eventData.add((int)ePlayer);
		postEvent(eventData);
	}
}

void CvDllPythonEvents::reportTraitLost(TraitTypes eTrait, PlayerTypes ePlayer)
{
	if (preEvent())
	{
		CyArgsList eventData;
		eventData.add("traitLost");						// add key to lookup python handler fxn
