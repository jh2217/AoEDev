...
void CvMapGenerator::addUniqueImprovements()
{
	...
	for (int iI = 0; iI < GC.getNumImprovementInfos(); iI++)
	{
		if (GC.getImprovementInfo((ImprovementTypes)iI).isUnique())
		{
			iBestValue = 0;
			pBestPlot = NULL;
			if (GC.getGameINLINE().isOption(GAMEOPTION_ALL_UNIQUE_IMPROVEMENTS) || GC.getGameINLINE().getSorenRandNum(100, "Unique Improvements") < iChance)
			{
...
/*************************************************************************************************/
/**	Orbis	why should adding improvement remove features?	19/09/09	Ahwaric		**/
/*************************************************************************************************/
/**	---- Start Original Code ----	**
