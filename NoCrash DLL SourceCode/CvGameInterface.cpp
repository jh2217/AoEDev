...
void CvGame::updateColoredPlots()
{
	...
	gDLL->getEngineIFace()->clearAreaBorderPlots(AREA_BORDER_LAYER_TARGETED_SPELL);
	gDLL->getEngineIFace()->clearAreaBorderPlots(AREA_BORDER_LAYER_TARGETED_SPELL_HIGHLIGHT);
...
/************************************************************************************************/
	...
	else if (pHeadSelectedUnit != NULL)
	{
		...
		{	
			...
			{
				for (iDX = -(iSpellRange); iDX <= iSpellRange; iDX++)
				{
					for (iDY = -(iSpellRange); iDY <= iSpellRange; iDY++)
					{
						CvPlot* pTargetPlot = plotXY(pHeadSelectedUnit->getX_INLINE(), pHeadSelectedUnit->getY_INLINE(), iDX, iDY);

						if (pTargetPlot != NULL && pHeadSelectedUnit->canCast((SpellTypes)iI,false, pTargetPlot))
						{
							if (pHeadSelectedUnit->canSpellTargetPlot(pTargetPlot,iI))
							{
								NiColorA color(GC.getColorInfo((ColorTypes)GC.getInfoTypeForString("COLOR_DARK_PURPLE")).getColor());
								color.a = 0.5f;
								gDLL->getEngineIFace()->fillAreaBorderPlot(pTargetPlot->getX_INLINE(), pTargetPlot->getY_INLINE(), color, AREA_BORDER_LAYER_TARGETED_SPELL);
							}
						}
					}
				}
...
/**																								**/
/**				Prevents highlighting Lairs which the unit cannot reach (Shipwrecks)			**/
/*************************************************************************************************/
/**								---- Start Original Code ----									**
...
/*************************************************************************************************/
/**	New Tag Defs	(ControlInfos)			02/01/09								Xienwolf	**/
/**																								**/
/**					Adds new Control Types (screens and other interface actions)				**/
/*************************************************************************************************/
...
/**																								**/
/**					Temporary fix for instant CtD on pressing these Hotkeys						**/
/*************************************************************************************************/
/**								---- Start Original Code ----									**
		if (gDLL->getInterfaceIFace()->isCityScreenUp())
		{
...
/*************************************************************************************************/
	case CONTROL_GUILD_SCREEN:
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "showGuildScreen");
		break;
/*************************************************************************************************/
/**	New Tag Defs							END													**/
/*************************************************************************************************/
...
ColorTypes CvGame::getPlotHighlightColor(CvPlot* pPlot) const
{
	...
	if (pPlot != NULL)
	{
		eColor = (ColorTypes) GC.getInfoTypeForString("COLOR_POSITIVE_TEXT");
		if (!gDLL->GetWorldBuilderMode())
		{
			...
			case INTERFACEMODE_TARGETED_SPELL:
				...
				{
					eColor = (ColorTypes)GC.getInfoTypeForString("COLOR_CLEAR");
				}
				else
				{
					eColor = (ColorTypes)GC.getInfoTypeForString("COLOR_DARK_GREY");
...
/*************************************************************************************************/
	ReligionTypes eCurRel = GET_PLAYER(getActivePlayer()).getStateReligion();
	if (eCurRel != NO_RELIGION)
	{
		CvReligionInfo& kCurrentRel = GC.getReligionInfo(eCurRel);
		if (kCurrentRel.getNumSoundtracks() > 0)
		{
			return kCurrentRel.getSoundtracks(GC.getASyncRand().get(kCurrentRel.getNumSoundtracks(), "Pick Song ASYNC"));
		}
	}
/*************************************************************************************************/
...
void CvGame::addSpellColoredPlot(CvPlot* pPlot, ColorTypes color, int range )
{
	gDLL->getEngineIFace()->clearColoredPlots(PLOT_LANDSCAPE_LAYER_RECOMMENDED_PLOTS);
	gDLL->getEngineIFace()->clearAreaBorderPlots(AREA_BORDER_LAYER_TARGETED_SPELL_HIGHLIGHT);
	int iDX, iDY;
	gDLL->getEngineIFace()->addColoredPlot(pPlot->getX_INLINE(), pPlot->getY_INLINE(), GC.getColorInfo(color).getColor(), PLOT_STYLE_DOT_TARGET, PLOT_LANDSCAPE_LAYER_RECOMMENDED_PLOTS);
	...
	for (iDX = -(range); iDX <= range; iDX++)
	{
		for (iDY = -(range); iDY <= range; iDY++)
		{
			//if (iDX != 0 || iDY != 0)
			//{
				gDLL->getEngineIFace()->fillAreaBorderPlot(pPlot->getX_INLINE() + iDX, pPlot->getY_INLINE() + iDY, GC.getColorInfo(color).getColor(), AREA_BORDER_LAYER_TARGETED_SPELL_HIGHLIGHT);

//			//}
		}
	}
}
void CvGame::clearSpellColoredPlot()
{
	gDLL->getEngineIFace()->clearColoredPlots(PLOT_LANDSCAPE_LAYER_RECOMMENDED_PLOTS);
	gDLL->getEngineIFace()->clearAreaBorderPlots(AREA_BORDER_LAYER_TARGETED_SPELL_HIGHLIGHT);
}
